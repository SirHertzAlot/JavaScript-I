...
const makeModel = inventory.find(is33);

//console.log(inventory.find(is33));
console.log(`Car 33 is a `+ makeModel.car_year+` `+makeModel.car_make +` `+makeModel.car_model);

// ==== Challenge 2 ====
// The dealer needs the information on the last car in their inventory.  What is the make and model of the last car in the inventory?  Log the make and model into the console.
const lastCar = inventory.find(is50)
...
const IsSorted = inventory.map(function (inventory) {
  
  return inventory.car_model;

});

console.log(IsSorted.sort());

// ==== Challenge 4 ====
// The accounting team needs all the years from every car on the lot. Create a new array from the dealer data containing only the car years and log the result in the console.
let carYears = [];

let years = inventory.map(function (inventory) {
...
var olderCars = years.filter(function (years) {
  return years < 2000;
});
oldCars.push(olderCars);
...
var BMW = inventory.filter(function (inventory) {
  return inventory.car_make === "BMW";
});

var Audi = inventory.filter(function (inventory) {
  return inventory.car_make === "Audi";
});

BMWAndAudi.push(BMW);
BMWAndAudi.push(Audi);
